...
function bundleWithBaseDir(basedir, options={}) {
  const {
    pathsDir,
    definitionsDir,
    componentsDir,
...
exports.bundle = function(options = {}) {
  if (Array.isArray(options.basedir)) {
    const specFiles = options.basedir.map((basedir) => bundleWithBaseDir(basedir, options));
    return _.merge({}, ...specFiles);
  } else {
    return bundleWithBaseDir(options.basedir, options);
  }
}
